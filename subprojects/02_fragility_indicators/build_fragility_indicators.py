...
def build_fragility_indicators(panel: pd.DataFrame) -> pd.DataFrame:
    ...
    Returns a DataFrame with columns [date, VolStress, SqStress, TailVolBreadth,
    TailLossBreadth, TailGainBreadth, AvgCorr60, AvgCorr30].
    """
    panel = panel.copy()
    ...
    q95_abs = abs_w.rolling(TAIL_WINDOW, min_periods=TAIL_MIN_PERIODS).quantile(0.95).shift(1)
    q05_ret = ret_w.rolling(TAIL_WINDOW, min_periods=TAIL_MIN_PERIODS).quantile(0.05).shift(1)
    q95_ret = ret_w.rolling(TAIL_WINDOW, min_periods=TAIL_MIN_PERIODS).quantile(0.95).shift(1)
    ...
    n_nan = result["TailVolBreadth"].isna().sum()
    log.info("  TailVolBreadth: %d NaN dates (warm-up), %d valid", n_nan, result["TailVolBreadth"].notna().sum())
...
def plot_fragility_timeline(fragility, agg_shocks, out_path):
    ...
    axes[0].plot(dates, fragility["VolStress"], color="#1f77b4", lw=0.8, label="VolStress")
    axes[0].set_ylabel("VolStress", fontsize=9)
    axes[0].set_title("Average absolute return (VolStress)", fontsize=9)
    _shade(axes[0]); axes[0].legend(fontsize=7)

    axes[1].plot(dates, fragility["TailVolBreadth"],  color="#2ca02c", lw=0.8, label="TailVolBreadth")
    axes[1].plot(dates, fragility["TailLossBreadth"], color="#d62728", lw=0.8, alpha=0.7, label="TailLossBreadth")
    axes[1].set_ylabel("Count (0-19)", fontsize=9)
    axes[1].set_title("Tail co-exceedance breadth", fontsize=9)
    axes[1].set_ylim(-0.5, N_COUNTRIES + 0.5)
    _shade(axes[1]); axes[1].legend(fontsize=7)

    axes[2].plot(dates, fragility["AvgCorr60"], color="#9467bd", lw=0.9, label="AvgCorr60")
    axes[2].plot(dates, fragility["AvgCorr30"], color="#c5b0d5", lw=0.7, alpha=0.8, label="AvgCorr30")
    axes[2].set_ylabel("Correlation", fontsize=9)
    axes[2].set_title("Rolling average pairwise correlation", fontsize=9)
    axes[2].set_ylim(-0.1, 1.0)
    _shade(axes[2]); axes[2].legend(fontsize=7)

    axes[3].bar(pd.to_datetime(agg_shocks["date"]), agg_shocks["max_shock"],
                color="#ff7f0e", width=1, alpha=0.7, label="MaxShock")
    axes[3].set_ylabel("S = -log(q)", fontsize=9)
    axes[3].set_title("Daily maximum shock intensity (MaxShock)", fontsize=9)
    _shade(axes[3]); axes[3].legend(fontsize=7)
...
def main():
    ...
    if not IN_PANEL.exists():
        raise FileNotFoundError(str(IN_PANEL) + " not found. Run SP01 first.")
